...
The end-to-end deep learning system was built on combining the feature extractors (contextual, word-level and character-level) combined with a learning component.
We selected three important feature types to represent the task's most relevant aspects of the data.
Transformer models, specifically RoBERTa and BioMed-RoBERTa, were employed to extract contextual features from each sample.
...
POS features were either one-hot encoded or transformed using bidirectional LSTM to encode long-term dependencies further and learn a task-specific grammatical structure from the input samples.
Contextual and word features were concatenated. 


Character orthographic features encapsulate capitalization, punctuation, word shape, and other orthographic features; e.g., "SemEval 2023!" encodes as "CccCccc nnnnp". 
A character-level token encoding is 28 dimensions, with a maximum token length of 20 characters applying post-padding on shorter words and truncating the longer ones.
These orthographic features were transformed using a 1-dimensional convolutional network (CNN) and fed to a fully-connected layer followed by either max pooling or global average pooling (GAP).
Post transformation, both transformer + word features and CNN transformed features were concatenated and fed to a linear layer to predict the entity sequence.
%
%
%
