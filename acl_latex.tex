...
Orthographic features encapsulate capitalization, punctuation, word shape, and other orthographic features; e.g., "SemEval 2023!" was encoded as "CccCccc nnnnp". 
Character features are sparser matrices encapsulating character-level information, including alphabet and individual punctuations.
...
These orthographic and character features were transformed using a 1-dimensional convolutional neural network (1D-CNN) and fed to a fully-connected (fc) layer followed by either max pooling (MAP) or global average pooling (GAP).
...
Each run averaged over the three most common random seeds in python, 0,1 and 42.
For subtask 2, we submitted the runs of the following model ensembles and architecture described in Section \ref{sec:system2}
%
\begin{enumerate}
    \item Run 1: Ensemble 6 RoB  
    \item Run 2: Ensemble 7 RoB  
    \item Run 3: Ensemble 13 RoB  
    \item Run 4: Ensemble 14 RoB  
