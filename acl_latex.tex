...
\textbf{Orthographic features} encapsulate capitalization, punctuation, word shape, and other orthographic features; e.g., "SemEval 2023!" was encoded as "CccCccc nnnnp". 
\textbf{Character features} are sparser matrices encapsulating character-level information, including alphabet and individual punctuations.
...
Post transformation, both transformer concatenated word features and CNN-transformed character features were concatenated and fed to a \textbf{linear layer} to predict the entity sequence.
%
%
%
...
\label{exps:task2}
%
For subtask 2, we submitted four runs, each an ensemble for PIO classes, and with the following architecture whose components are described in Section \ref{sec:system2}.
...
%
\begin{enumerate}
    \item Run 1: ( transformer (RoBERTa) $\oplus$ POS (BiLSTM) ) $\oplus$ ( orthographic features - 1D-CNN - fc  ) (linear layrer)
    \item Run 2: ( transformer (RoBERTa) $\oplus$ POS (one-hot) ) $\oplus$ ( character features - 1D-CNN - fc  ) (linear layrer)
    \item Run 3: ( transformer (RoBERTa) $\oplus$ POS (one-hot) ) $\oplus$ ( orthographic features - 1D-CNN - fc - GAP ) (linear layrer) 
    \item Run 4: ( transformer (RoBERTa) $\oplus$ POS (BiLSTM) ) $\oplus$ ( orthographic features - 1D-CNN - fc - GAP ) (linear layrer) 
