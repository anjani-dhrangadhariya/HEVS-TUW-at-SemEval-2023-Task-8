...
\item distilBERT$_{pe}$ -- distilBERT model traind on personal experience entities %: st1\_test\_flair\_per\_exp.csv
\item distilBERT$_c$ -- distilBERT model traind on claim entities %: st1\_test\_flair\_per\_exp.csv
\end{enumerate}
...
\label{system:task2}
%
PIO extraction was planned as a two-part system: First part combined a text preprocessing and enriching module, and the second part was an end-to-end deep learning entity extraction pipeline.
Explain the preprocessing steps here: removing trailing spaces, lower casing, etc. %Anjani
After preprocessing, text tokens were enriched with the part-of-the-speech tags and token lemmas using spaCy~\footnote{https://spacy.io/}. 


The end-to-end deep learning system was built on combining the feature extractors (contextual, word-level and character-level) and a learning component.
We selected three important feature types to represent the most relevant aspects of the data.
Transformer models, specifically RoBERTa and BioMed-RoBERTa, were employed to extract contextual features from input samples.
The word-level features are 30-dimensional vectors comprising POS tags generated by spaCy. 
POS tags help NLP models better understand the syntactic structure of a sentence.
POS features were either one-hot encoded or transformed using bidirectional LSTM (Long short-term memory) to encode long-term dependencies and learn a task-specific grammatical structure from the input samples.
Contextual and word features were concatenated. 


Character orthographic features encapsulate capitalization, punctuation, word shape, and other orthographic features; e.g., "SemEval 2023!" was encoded as "CccCccc nnnnp". 
A character-level token encoding was 28 dimensional, with a maximum token length of 20 characters applying post-padding on shorter words and truncating the longer ones.
These orthographic features were transformed using a 1-dimensional convolutional neural network (CNN) and fed to a fully-connected layer followed by either max pooling or global average pooling (GAP).
Post transformation, both transformer concatenated word features and CNN transformed character features were concatenated and fed to a linear layer to predict the entity sequence.
%
%
%
